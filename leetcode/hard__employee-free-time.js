...
var employeeFreeTime = function (schedule) {
  ...
  for (let i = 0; i < schedule.length; i++) {
    const employeeShifts = schedule[i];
    for (let j = 0; j < employeeShifts.length; j++) {
      const shift = employeeShifts[j];
      starts.push(shift[0]);
      ends.push(shift[1]);
    }
  }
  ...
  for (let i = 1; i < sortedStarts.length; i++) {
    if (sortedStarts[i] > sortedEnds[i - 1]) {
      results.push([sortedEnds[i - 1], sortedStarts[i]]);
    }
  }
  return results;
};
